...
canWeDeliver = zipCode => {
    let found = deliveryAreaZipCodes.includes(zipCode)
    if (found === true) {
        return `Addresses located in ${zipCode} are in our delivery zone!`
    } else {
        return `Sorry, we can't deliver to addresses located in ${zipCode}`
    }
}
...
canWeDeliverTwo = zipCode => {
    ...
    if (result === false) {
        return `Sorry, we are not delivering to addresses located in ${zipCode} at this time.`
    }
}
